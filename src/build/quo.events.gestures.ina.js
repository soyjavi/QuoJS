// NOT!!! Generated by CoffeeScript 1.3.3
  ...
  (function($$) {
    ...
    _onTouchEnd = function(event) {
      ...
      if(gesture_data.fingers === 1) {
        events.swipeX && _trigger("swipe" + events.swipeX);
        events.swipeY && _trigger("swipe" + events.swipeY);
        ...
        if(events.rotate) {
          ...
          _trigger("rotate" + events.pinch, {angle: gesture_data.last_angle});
        }
      }
    ...
    _captureRotation = function() {
      // esta funcion hay que cambiarla porque esta muy guarrete hecho
      var angle = _angle(touches2);
      var diff = parseInt(gesture_data.start_angle - angle, 10);
      if(Math.abs(diff) > 5 || gesture_data.last_angle != 0) {
        if(Math.abs(diff-gesture_data.last_angle) > 90) {
          var symbol = gesture_data.last_angle < 0 ? '-' : '+';
          while(Math.abs(diff - gesture_data.last_angle) >= 90) {
    ...
    _angle = function(touches_data) {
      ...
      return parseInt((angle<0 ? angle+180 : angle), 10);
    };
